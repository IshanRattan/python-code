...
class MultiHeadAttention(Layer):
    ...
    def call(self, queries : tf.Tensor,
        ...
        # attention dim after the line below : (batch_size, MAX_LEN, nb_proj, d_proj)
        attention = tf.transpose(attention, perm=[0, 2, 1, 3])

        # concat_attention dim : (batch_size, MAX_LEN, d_model)
        # -1 flattens all the elements into a 1-d array
        # reshape re-arranges the data into:
        #  (batch_size, (queries.batch_size * queries.MAX_LENGTH * queries.d_model) / d_model, d_model)
...
def scaled_dot_product_attention(queries : tf.Tensor,
    ...
    # queries/keys tensor is basically, (batch_size, nb_proj(attention_heads), MAX_LENGTH, d_model // attention_heads)
    # Matrix multiplication happens with (MAX_LENGTH, d_model // attention_heads) for both the matrices
    # which is (20, 16) * transpose((20, 16)) and result is a (20, 20) matrix
