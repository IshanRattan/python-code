...
def remove_spl_chars(sentences):
    clean_sentences = []
    spl_chars = ['.', ',', '?', '(', ')']
    for sentence in sentences:
        ...
        clean_sentences.append(sentence.replace('-', ' ').lower().strip())
    return clean_sentences

def get_tokenizers(input_lang_sents : list, target_lang_sents : list):

    tokenizer_input_lang = tfds.deprecated.text.SubwordTextEncoder.build_from_corpus(input_lang_sents,
                                                                                     target_vocab_size=2 ** 13)
    tokenizer_target_lang = tfds.deprecated.text.SubwordTextEncoder.build_from_corpus(target_lang_sents,
                                                                                      target_vocab_size=2 ** 13)
    return tokenizer_input_lang, tokenizer_target_lang

def build_training_data() -> (tf.data.Dataset, int, int):
    ...
    tokenizer_input_lang, tokenizer_target_lang = get_tokenizers(input_language_preprocessed, target_language_preprocessed)
